...
class LockPanel(tk.Frame):
    ...
    def __init__(self, app):
        ...
        self._pulse.pack(pady=(10, 0))

        tk.Label(card, text="SESSION LOCKED", font=("Courier", 14, "bold"),
                 bg=C["bg_card"], fg=C["text_white"]).pack(pady=(2, 1))
        tk.Label(card, text="Enter master password to unlock", font=("Courier", 10),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(pady=(0, 10))

        tk.Label(card, text="MASTER PASSWORD", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=40)
        self._pwd_entry = DarkEntry(card, placeholder="password", show="•", width=20)
        self._pwd_entry.pack(padx=40, pady=(2, 8))
        self._pwd_entry.bind_key("<Return>", lambda _: self._unlock())

        self._status = tk.Label(card, text="", font=("Courier", 10),
        ...
        NeonButton(btn_row, text="UNLOCK", colour=C["neon_violet"],
                   command=self._unlock).pack(side="left", ipadx=15, ipady=5, padx=(0, 8))
        NeonButton(btn_row, text="LOGOUT", colour=C["neon_orange"],
                   command=self._logout).pack(side="left", ipadx=10, ipady=5)
