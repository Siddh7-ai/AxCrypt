...
class SettingsPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        left.pack(side="left", fill="both", expand=True, padx=(12, 5), pady=(0, 10))
        right = tk.Frame(self, bg=C["bg_deep"])
        right.pack(side="right", fill="both", expand=True, padx=(0, 12), pady=(0, 10))
        ...
        tk.Label(sess_card, text="⏱️  SESSION SETTINGS",
                 ...
                 bg=C["bg_card"], fg=C["neon_cyan"]).pack(anchor="w", padx=15, pady=(8, 5))

        # Auto-lock timeout
        tk.Label(sess_card, text="AUTO-LOCK TIMEOUT (seconds)",
                 font=("Courier", 9), bg=C["bg_card"], fg=C["text_dim"]
        ...
        timeout_entry = tk.Entry(sess_card, textvariable=self._timeout_var,
                                 font=("Courier", 11), bg=C["bg_input"],
                                 fg=C["text_hi"], insertbackground=C["neon_cyan"],
                                 relief="flat", bd=4, highlightthickness=1,
                                 highlightcolor=C["neon_cyan"],
                                 highlightbackground=C["border"], width=10)
        timeout_entry.pack(anchor="w", padx=15, pady=(2, 3))
        ...
        chg_card.pack(fill="x", pady=(0, 8))
        tk.Label(chg_card, text="🔐  CHANGE PASSWORD",
                 font=("Courier", 11, "bold"),
                 bg=C["bg_card"], fg=C["neon_orange"]).pack(anchor="w", padx=15, pady=(8, 5))
        tk.Label(chg_card, text="CURRENT PASSWORD", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=15)
        self._chg_old = DarkEntry(chg_card, placeholder="current", show="•", width=56)
        self._chg_old.pack(padx=15, pady=(2, 3))
        tk.Label(chg_card, text="NEW PASSWORD", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=15)
        self._chg_new = DarkEntry(chg_card, placeholder="new password", show="•", width=56)
        self._chg_new.pack(padx=15, pady=(2, 3))
        tk.Label(chg_card, text="CONFIRM NEW", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=15)
        self._chg_confirm = DarkEntry(chg_card, placeholder="confirm", show="•", width=56)
        self._chg_confirm.pack(padx=15, pady=(2, 3))
        self._chg_status = tk.Label(chg_card, text="", font=("Courier", 10),
                                    bg=C["bg_card"], fg=C["danger"])
        self._chg_status.pack(pady=(2, 3))
        NeonButton(chg_card, text="CHANGE PASSWORD", colour=C["neon_orange"],
                   command=self._do_change_pwd).pack(pady=(2, 8), ipadx=12, ipady=4)
        ...
        tk.Label(tl_card, text="⏰  TIME-LOCK PASSWORD GENERATOR",
                 font=("Courier", 11, "bold"),
                 bg=C["bg_card"], fg=C["neon_green"]).pack(anchor="w", padx=15, pady=(8, 5))

        tk.Label(tl_card, text="BASE PASSWORD", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=15)
        self._tl_pwd = DarkEntry(tl_card, placeholder="password to wrap", show="•", width=56)
        self._tl_pwd.pack(padx=15, pady=(2, 3))

        tk.Label(tl_card, text="DURATION", font=("Courier", 9),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=15)
        dur_row = tk.Frame(tl_card, bg=C["bg_card"])
        dur_row.pack(fill="x", padx=15, pady=(2, 8))
        ...
        for label, val in dur_options:
            tk.Radiobutton(dur_row, text=label, variable=self._dur_var, value=val,
                           ...
                           selectcolor=C["bg_input"], font=("Courier", 9)
                           ).pack(side="left", padx=(0, 4))

        self._tl_output = tk.Label(tl_card, text="", font=("Courier", 9),
                                   bg=C["bg_card"], fg=C["neon_green"],
                                   justify="left", wraplength=380)
        self._tl_output.pack(anchor="w", padx=15, pady=(3, 3))

        btn_row = tk.Frame(tl_card, bg=C["bg_card"])
        btn_row.pack(pady=(2, 8))
        NeonButton(btn_row, text="GENERATE", colour=C["neon_green"],
                   command=self._gen_time_lock).pack(side="left", ipadx=10, ipady=4, padx=(0, 8))
        self._copy_btn = NeonButton(btn_row, text="COPY", colour=C["neon_cyan"],
                                    command=self._copy_token)
        self._copy_btn.pack(side="left", ipadx=10, ipady=4)
        ...
        self._about_text.pack(fill="both", expand=True, padx=30, pady=(0, 8))
