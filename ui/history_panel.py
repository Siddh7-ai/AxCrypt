...
class HistoryPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text="📜  ENCRYPTION HISTORY",
                 ...
                 bg=C["bg_panel"], fg=C["text_white"]).pack(side="left", padx=(15, 0), pady=12)
        ...
        NeonButton(btn_row, text="🔍 VERIFY CHAIN", colour=C["neon_violet"],
                   command=self._verify_chain).pack(side="left", ipadx=10, ipady=4, padx=(0, 8))
        NeonButton(btn_row, text="🗑️ CLEAR", colour=C["danger"],
                   command=self._clear_history).pack(side="left", ipadx=10, ipady=4)

        # Filter row
        filter_row = tk.Frame(self, bg=C["bg_panel"])
        filter_row.pack(fill="x", padx=15, pady=(5, 3))
        tk.Label(filter_row, text="FILTER:", font=("Courier", 10),
        ...
        for label, value in [("ALL", ""), ("ENCRYPT", "ENCRYPT"),
            ...
            btn = tk.Button(filter_row, text=label, font=("Courier", 9, "bold"),
                            bg=bg, fg=fg, activebackground=C["border"],
                            activeforeground=C["text_hi"],
                            relief="flat", bd=0, cursor="hand2",
                            command=lambda v=value: self._apply_filter(v))
            btn.pack(side="left", padx=(6, 0), pady=9, ipadx=19, ipady=4)

        # Chain-integrity status
        self._chain_status = tk.Label(self, text="", font=("Courier", 10, "bold"),
                                      bg=C["bg_deep"], fg=C["text_dim"])
        self._chain_status.pack(pady=(2, 4))

        # ── table header ───────────────────────────────────────────────────
        self._table_frame = tk.Frame(self, bg=C["bg_deep"])
        self._table_frame.pack(fill="both", expand=True, padx=15, pady=(0, 8))
    ...
    def _header_row(self):
        cols = [("No.", 102), ("#  TIME", 460), ("ACTION", 256),
                ("FILE", 716), ("STATUS", 230), ("USER", 307)]
        row = tk.Frame(self._scroll_frame, bg=C["bg_panel"])
        row.pack(fill="x", pady=(0, 2))
        for txt, w in cols:
            tk.Label(row, text=txt, font=("Courier", 9, "bold"),
    ...
    def _populate(self, filter_action: str = ""):
        ...
        if not entries:
            tk.Label(self._scroll_frame, text="  No history entries.",
                     font=("Courier", 10), bg=C["bg_deep"], fg=C["text_dim"]
                     ).pack(pady=51, anchor="w")
            return
        ...
        for i, entry in enumerate(entries):
            ...
            tk.Label(row, text=str(i + 1), font=("Courier", 10),
                     bg=row_bg, fg=C["text_dim"], width=5, anchor="w"
                     ).pack(side="left", padx=(8, 0))
            tk.Label(row, text=ts, font=("Courier", 10),
                     bg=row_bg, fg=C["text_mid"], width=18, anchor="w"
                     ).pack(side="left", padx=(4, 0))
            tk.Label(row, text=action, font=("Courier", 10, "bold"),
                     bg=row_bg, fg=a_colour, width=10, anchor="w"
                     ).pack(side="left", padx=(4, 0))
            tk.Label(row, text=fname, font=("Courier", 10),
                     bg=row_bg, fg=C["text_hi"], width=56, anchor="w"
                     ).pack(side="left", padx=(4, 0))
            tk.Label(row, text=status, font=("Courier", 10, "bold"),
                     bg=row_bg, fg=s_colour, width=4, anchor="w"
                     ).pack(side="left", padx=(4, 0))
            tk.Label(row, text=user, font=("Courier", 10),
                     bg=row_bg, fg=C["text_dim"], width=12, anchor="w"
                     ).pack(side="left", padx=(4, 0))
