...
class DashboardPanel(tk.Frame):
    ...
    def __init__(self, app):
        ...
        top.pack(fill="x", padx=15, pady=(10, 8))

        # Welcome card (wider)
        self._welcome_card = CardFrame(top, border_colour=C["neon_cyan"])
        self._welcome_card.pack(side="left", fill="both", expand=True, padx=(0, 8))
        self._welcome_text = TerminalText(self._welcome_card)
        self._welcome_text.pack(fill="both", expand=True, ipady=25)

        # Stats column
        stats_col = tk.Frame(top, bg=C["bg_deep"], width=200)
        ...
        for label, key, colour in [
            ...
            tk.Label(card, text=label, font=("Arial", 9),
                     bg=C["bg_card"], fg=C["text_dim"]).pack(pady=(6, 0))
            lbl = tk.Label(card, text="0", font=("Arial", 22, "bold"),
        ...
        mid.pack(fill="x", padx=15, pady=(0, 8))

        # Difficulty card
        diff_card = CardFrame(mid, border_colour=C["neon_orange"])
        diff_card.pack(side="left", fill="both", expand=True, padx=(0, 8), ipady=19)
        tk.Label(diff_card, text="⚡ ENCRYPTION DIFFICULTY", font=("Arial", 11, "bold"),
                 bg=C["bg_card"], fg=C["neon_orange"]).pack(pady=(8, 4))
        self._diff_label = tk.Label(diff_card, text="─", font=("Arial", 18, "bold"),
        ...
        NeonButton(qa_card, text="HISTORY", colour=C["neon_violet"],
                   ...
                   ).pack(pady=(4, 14), padx=12, ipadx=12, ipady=5)

        # ── bottom: recent activity feed ──────────────────────────────────
        bot = tk.Frame(self, bg=C["bg_deep"])
        bot.pack(fill="both", expand=True, padx=15, pady=(0, 6))
        recent_card = CardFrame(bot, border_colour=C["border_glow"])
        recent_card.pack(fill="both", expand=True)
        tk.Label(recent_card, text="📋  RECENT ACTIVITY", font=("Arial", 11, "bold"),
                 bg=C["bg_card"], fg=C["text_mid"]).pack(anchor="w", padx=40, pady=(8, 4))
