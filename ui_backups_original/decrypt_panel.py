...
class DecryptPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # ── LEFT: controls ────────────────────────────────────────────────
        left = tk.Frame(self, bg=C["bg_deep"], width=736)
        left.pack(side="left", fill="y", padx=(18, 8), pady=28)
        ...
        tk.Label(file_card, text="📂  SELECT ENCRYPTED FILE",
                 font=("Arial", 27, "bold"),
                 bg=C["bg_card"], fg=C["neon_cyan"]).pack(anchor="w", padx=25, pady=(12, 6))
        ...
        NeonButton(btn_row, text="BROWSE", colour=C["neon_cyan"],
                   command=self._browse_file).pack(side="left", ipadx=22, ipady=6)
        ...
        tk.Label(self._steg_card, text="🕵️  STEGANOGRAPHIC METADATA",
                 font=("Arial", 25, "bold"),
                 bg=C["bg_card"], fg=C["neon_violet"]).pack(anchor="w", padx=25, pady=(10, 4))
        self._steg_label = tk.Label(self._steg_card, text="No hidden data found.",
                                    font=("Arial", 22), bg=C["bg_card"], fg=C["text_dim"],
                                    justify="left", wraplength=400)
        self._steg_label.pack(anchor="w", padx=25, pady=(0, 10))

        # Password
        pwd_card = CardFrame(left, border_colour=C["neon_green"])
        pwd_card.pack(fill="x", pady=(0, 10))
        tk.Label(pwd_card, text="🔑  PASSWORD", font=("Arial", 27, "bold"),
                 bg=C["bg_card"], fg=C["neon_green"]).pack(anchor="w", padx=25, pady=(12, 6))
        tk.Label(pwd_card, text="DECRYPTION PASSWORD (or time-locked token)",
                 font=("Arial", 19), bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=25)
        self._pwd_entry = DarkEntry(pwd_card, placeholder="password / token", width=67)
        self._pwd_entry.pack(padx=25, pady=(2, 6))

        # Token validator
        self._token_status = tk.Label(pwd_card, text="", font=("Arial", 19),
                                      bg=C["bg_card"], fg=C["text_dim"])
        self._token_status.pack(padx=25, pady=(0, 4))
        self._pwd_entry.bind_key("<KeyRelease>", self._check_token)

        # OTD warning
        self._otd_warning = tk.Label(left, text="", font=("Arial", 22, "bold"),
                                     bg=C["bg_deep"], fg=C["neon_pink"])
        self._otd_warning.pack(pady=(2, 4))
        ...
        tk.Label(del_row, text="🗑️ Secure-delete decrypted file after viewing",
                 font=("Arial", 22), bg=C["bg_deep"], fg=C["neon_orange"]).pack(side="left")

        # Status + progress
        self._status = tk.Label(left, text="", font=("Arial", 22),
                                bg=C["bg_deep"], fg=C["danger"])
        self._status.pack(pady=(4, 4))
        self._progress = NeonProgressBar(left, width=684, colour=C["neon_cyan"])
        self._progress.pack(pady=(0, 4))
        self._progress_label = tk.Label(left, text="", font=("Arial", 19),
        ...
        self._decrypt_btn.pack(pady=(8, 0), ipadx=64, ipady=12)

        # ── RIGHT: live viz ───────────────────────────────────────────────
        right = tk.Frame(self, bg=C["bg_deep"])
        right.pack(side="right", fill="both", expand=True, padx=(0, 18), pady=28)
        ...
        stages_frame.pack(fill="x", padx=25, pady=(0, 8))
        self._stage_labels = {}
        self._stage_bars   = {}
        stages = [
        ...
        for i, (name, colour, desc) in enumerate(stages):
            ...
            circle = tk.Label(row, text="●", font=("Arial", 35),
                              bg=C["bg_card"], fg=C["text_dim"])
            circle.pack(side="left", padx=(4, 8))
            lbl = tk.Label(row, text=name, font=("Arial", 25, "bold"),
                           bg=C["bg_card"], fg=C["text_dim"])
            lbl.pack(side="left")
            sub = tk.Label(row, text=f"  [{desc}]", font=("Arial", 19),
                           bg=C["bg_card"], fg=C["text_dim"])
            sub.pack(side="left")
            bar_bg = tk.Frame(row, bg=C["bg_input"], width=192, height=6)
        ...
        tk.Frame(viz_card, bg=C["border"], height=1).pack(fill="x", padx=25, pady=12)
        tk.Label(viz_card, text="BYTE STREAM", font=("Arial", 19),
                 bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=25)
        self._byte_stream = TerminalText(viz_card)
        self._byte_stream.pack(fill="both", expand=True, padx=19, pady=(2, 12))
    ...
    def _run_decrypt(self, password: str):
        ...
        try:
            ...
            self.ui_queue.put(("stage", 12, "Deriving key …"))
            time.sleep(0.5)

            # Stage 1: Header parse
            self.ui_queue.put(("stage", 12, "Parsing file header …"))
            time.sleep(0.3)

            # Stage 2: Decryption
            self.ui_queue.put(("stage", 12, "Decrypting …"))
            ...
            # Stage 3: Output
            self.ui_queue.put(("stage", 12, "Writing output …"))
            time.sleep(0.3)
