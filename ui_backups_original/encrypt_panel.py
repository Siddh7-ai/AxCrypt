...
class EncryptPanel(tk.Frame):
    ...
    def _build_ui(self):
        ...
        tk.Label(file_card, text="📂  SELECT FILE", font=("Arial", 27, "bold"),
        ...
        tk.Label(pwd_card, text="PASSWORD", font=("Arial", 19),
                bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=25)
        self._pwd_entry = DarkEntry(pwd_card, placeholder="encryption password", show="•", width=67)
        self._pwd_entry.pack(padx=25, pady=(2, 6))
        self._pwd_entry.bind_key("<KeyRelease>", self._on_pwd_change)

        tk.Label(pwd_card, text="CONFIRM PASSWORD", font=("Arial", 19),
                bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=25)
        self._pwd_confirm = DarkEntry(pwd_card, placeholder="confirm password", show="•", width=67)
        self._pwd_confirm.pack(padx=25, pady=(2, 4))

        self._strength = StrengthBar(pwd_card)
        self._strength.pack(padx=25, pady=(2, 10), fill="x")

        # Options
        opt_card = CardFrame(left, border_colour=C["neon_violet"])
        opt_card.pack(fill="x", pady=(0, 10))
        tk.Label(opt_card, text="⚙️  OPTIONS", font=("Arial", 27, "bold"),
        ...
        otd_row.pack(fill="x", padx=25, pady=(0, 6))
        ...
        tk.Label(otd_row, text="🔥 One-Time Decrypt", font=("Arial", 22, "bold"),
                bg=C["bg_card"], fg=C["neon_pink"]).pack(side="left")

        tk.Label(opt_card, text="OWNER INFO (hidden)", font=("Arial", 19),
                bg=C["bg_card"], fg=C["text_dim"]).pack(anchor="w", padx=25)
        self._owner_entry = DarkEntry(opt_card, placeholder="your name", width=67)
        self._owner_entry.pack(padx=25, pady=(2, 12))

        # Status + progress
        self._status = tk.Label(left, text="", font=("Arial", 22),
                                bg=C["bg_deep"], fg=C["danger"])
        self._status.pack(pady=(4, 4))
        self._progress = NeonProgressBar(left, width=684, colour=C["neon_green"])
        ...
        self._encrypt_btn.pack(pady=(8, 0), ipadx=64, ipady=12)
        ...
        tk.Label(viz_card, text="📡  ENCRYPTION VISUALIZATION",
                font=("Arial", 30, "bold"),
                bg=C["bg_card"], fg=C["text_white"]).pack(pady=(14, 8))
        self._viz_log = TerminalText(viz_card)
        self._viz_log.pack(fill="both", expand=True, padx=19, pady=(2, 12))
